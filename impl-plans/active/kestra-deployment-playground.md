...
**Tasks Completed**: Created live GCP project `kestra-playground-260625`, enabled required APIs,
...
playground scope. Verified project `kestra-playground-260625` is active, the single-VM endpoint
...
**Validation**:
- `gcloud projects describe kestra-playground-260625`
...
in kinko, created GCP project `kestra-playground-260625`, attached billing, enabled required APIs,
...
**Validation**:
- `nix develop -c tofu apply -var=project_id=kestra-playground-260625` in `infra/terraform/gce-single`
...
- `nix develop -c uv run ty check`
- `nix develop -c uv run pytest`
- `nix develop -c tofu -chdir=infra/terraform/gce-single plan -detailed-exitcode -var=project_id=kestra-playground-260625`
  returned exit code 0, confirming no live single-VM infrastructure drift after the dependency
  hardening.

### Session: 2026-06-25 11:20

**Tasks Completed**: Used the Cloudflare zone `tacoserve.online` for live HTTPS DNS. Created and
...
- `kinko exec --env CLOUDFLARE_API_TOKEN -- curl https://api.cloudflare.com/client/v4/user/tokens/verify`
  returned success.
- DNS resolves:
  - `k8s.tacoserve.online` -> `107.178.251.34`
  - `gce-container.tacoserve.online` -> `34.149.73.29`
  - `gce-compose.tacoserve.online` -> `8.232.228.251`
- GKE `ManagedCertificate` is `Active` and `curl -fsSI https://k8s.tacoserve.online/ui/` returned
  HTTP 200.
- GCE Google-managed certificates `kestra-cluster-dev-https` and `kestra-dev-https` are `ACTIVE`.
- `curl -fsSI https://gce-container.tacoserve.online/ui/` returned HTTP 200.
- `curl -fsSI https://gce-compose.tacoserve.online/ui/` returned HTTP 200.
