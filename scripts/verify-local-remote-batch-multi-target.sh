...
TARGETS='[{"id":"worker-a","host":"remote-worker","port":22,"username":"batch","password_secret":"REMOTE_BATCH_PASSWORD","execute_failure_marker":""},{"id":"worker-b","host":"remote-worker-b","port":22,"username":"batch","password_secret":"REMOTE_BATCH_PASSWORD","execute_failure_marker":"/tmp/kestra-fail-execute-once"}]'

restore_second_worker() {
  "${COMPOSE[@]}" exec -T remote-worker-b rm -f "${FAILURE_MARKER}" >/dev/null 2>&1 || true
}
trap restore_second_worker EXIT
...
inject_second_worker_execute_failure() {
  "${COMPOSE[@]}" exec -T --user batch remote-worker-b touch "${FAILURE_MARKER}"
}

assert_no_remote_workspaces() {
  local worker

  for worker in remote-worker remote-worker-b; do
    # The workspace variables must expand inside the target container, not in this shell.
...
baseline_worker_a_id="$(correlated_execution_id "${baseline_parent_id}" remote_batch_runner worker-a)"
baseline_worker_b_id="$(correlated_execution_id "${baseline_parent_id}" remote_batch_runner worker-b)"
assert_child_step_attempts "${baseline_worker_a_id}" worker-a 1
assert_child_step_attempts "${baseline_worker_b_id}" worker-b 1
print_target_evidence \
  "Baseline multi-target execution ${baseline_multi_id}" \
...
retry_worker_a_id="$(correlated_execution_id "${retry_parent_id}" remote_batch_runner worker-a)"
retry_worker_b_id="$(correlated_execution_id "${retry_parent_id}" remote_batch_runner worker-b)"
assert_child_step_attempts "${retry_worker_a_id}" worker-a 1
assert_child_step_attempts "${retry_worker_b_id}" worker-b 2
print_target_evidence \
  "Isolated retry execution ${retry_multi_id}" \
  "${retry_worker_a_id}" \
  "${retry_worker_b_id}"
assert_no_remote_workspaces
...
echo "Verified two-target SSH fan-out and an isolated execute-task retry between one-attempt validation tasks."
