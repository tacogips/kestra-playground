...
The live development project created for this playground is `example-project-id`.
...
`gs://example-project-id-tofu-state`. The live roots use per-root prefixes so GitHub Actions
...
```bash
tofu apply \
  -var='project_id=example-project-id' \
